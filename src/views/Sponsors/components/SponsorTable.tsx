...
const ActionColumn = ({ row }: { row: Sponsor }) => {
...
const NameColumn = ({ row }: { row: Sponsor }) => {
    const { textTheme } = useThemeClass()
    ...
    return (
        <div className="flex w-28 items-center">
            ...
            <span className="ml-2 font-semibold">
                {row.firstName ?? 'Dummy Name'} {row.lastName}
            </span>
            {/* </Link> */}
        </div>
    )
...
const Customers = () => {
    const dispatch = useAppDispatch()
    const data = useAppSelector((state) => state.rwwSponsors.data.customerList)
    const loading = useAppSelector((state) => state.rwwSponsors.data.loading)
    const filterData = useAppSelector(
        (state) => state.rwwSponsors.data.filterData
    )
    ...
    // useEffect(() => {
    //     fetchData()
    // }, [fetchData, pageIndex, pageSize, filterData])
    ...
    return (
        <>
            <DataTable
                ...
                loading={loading}
                pagingData={{
                    total: tableData?.total ?? (0 as number),
