...
import { sponsorData } from '../data/usersData'
...
export default function crmFakeApi(server: Server, apiPrefix: string) {
    ...
    // ------------SPONSORS --------------------------//
    server.post(`${apiPrefix}/crm/sponsors`, (schema, { requestBody }) => {
        const body = JSON.parse(requestBody)
        // const { pageIndex, pageSize, sort, query } = body
        // const { order, key } = sort
        const users = schema.db.sponsorData
        // console.log(users)
        const sanitizeUsers = users.filter((elm) => typeof elm !== 'function')
        let data = sanitizeUsers
        let total = users.length
        ...
        // if (key && order) {
        //     if (key === 'dob') {
        //         data.sort((a, b) => dayjs(a.dob).diff(dayjs(b.dob)))
        //     }
        //     if (key !== 'lastOnline') {
        //         data.sort(
        //             sortBy(key, order === 'desc', (a) =>
